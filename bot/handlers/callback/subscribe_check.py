...
async def subscribe_check_false(query: types.CallbackQuery, user: dict, db_worker: DBWorker, _: dict):
    ...
    await query.message.answer(_["subscribe_groups"], reply_markup=subscribe(chat_name_url, _['check_subscribe_button']))

async def subscribe_check_true(query: types.CallbackQuery, user: dict, db_worker: DBWorker, _: dict):
